...
Boolean (true или false). ћетод Уempty?Ф используетс€ дл€ того, чтобы убедитьс€ в том,
что массив не пустой (или пустой). ≈сли массив пустой (empty), то Уempty?Ф возвращает
УtrueФ:
=end
...
=begin
¬ажный момент заключаетс€ в том, что объект УnilФ не реализует метод Уempty?Ф. “.е.
если вы не уверены, что какой-то метод возвращает массив, необходимо сделать
проверку на nil:
=end
...
=begin
—уществует одна важна€ деталь. “.к. любой руби-программист почти со 100%
веро€тностью будет работать с rails, нужно знать, что проверка коллекции (в т.ч.массива)
в rails выполн€етс€ иначе. “.е. если вы оставите этот синтаксис, то ошибки не будет,
просто есть более эффективный способ:
=end
...
=begin
ћетод count выполн€ет ту же функцию, что и length/size, но только дл€ классов Array и
Hash (не реализован в String). ќднако, метод count может принимать блок, можно
использовать его дл€ каких-либо вычислений. Ќапример, посчитать количество нулей в
массиве:
=end
...
=begin
”добно использовать метод count вместе с указателем на функцию. ≈сли метод Уzero?Ф
...
=begin
«адание: с помощью указател€ на функцию посчитайте количество четных элементов в
